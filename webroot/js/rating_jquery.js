...
function ratingSet(element, value) {
  var starImg = ratingSettings[element]['config']['statEmptyClass'];
  
  for (i = 1; i <= ratingSettings[element]['data']['%MAX%']; i++) {
    if (i <= Math.floor(value)) {
      starImg = ratingSettings[element]['config']['statFullClass'];
    } else if (i == Math.floor(value) + 1 && value.toString().match(/[0-9]\.[5-9]/)) {
      starImg = ratingSettings[element]['config']['statHalfClass'];
    } else {
      starImg = ratingSettings[element]['config']['statEmptyClass'];
    }
    
    $('#' + element + '_' + i).attr({'class': ratingSettings[element]['config']['appRoot'] + starImg});
